...
class SkeletonGenerator {
    ...
    async promptOptions() {
        try {
            const targetPath = await this.promptWithValidation(() => (0, prompts_1.input)({
                message: '生成先のパスを入力してください:',
                default: './my-new-project',
            }), (value) => {
                if (!value.trim()) {
                    throw new Error('パスを入力してください');
                }
                return value;
            });
            const projectName = await this.promptWithValidation(() => (0, prompts_1.input)({
                ...
                if (!/^[a-zA-Z0-9-_]+$/.test(value)) {
                    throw new Error('プロジェクト名は英数字、ハイフン、アンダースコアのみ使用可能です');
                }
                return value;
            });
            const includeDocs = await (0, prompts_1.confirm)({
                message: 'ドキュメントファイルを含めますか？',
                default: true,
            });
            const includeScripts = await (0, prompts_1.confirm)({
                message: 'スクリプトファイルを含めますか？',
                default: true,
            });
            const includePrompts = await (0, prompts_1.confirm)({
                message: 'プロンプトファイルを含めますか？',
                default: true,
            });
            const includeInfrastructure = await (0, prompts_1.confirm)({
                message: 'インフラストラクチャファイルを含めますか？',
                default: false,
            });
            const customCursorRules = await (0, prompts_1.confirm)({
            ...
            if (error instanceof Error) {
                throw new Error(`オプションの入力中にエラーが発生しました: ${error.message}`);
            }
            throw new Error('オプションの入力中に不明なエラーが発生しました');
        }
    }
    async promptWithValidation(promptFn, validateFn) {
        while (true) {
            try {
                ...
                if (error instanceof Error) {
                    ...
                    console.log(chalk_1.default.red('❌ 入力エラーが発生しました'));
                }
                console.log(chalk_1.default.gray('もう一度入力してください...\n'));
            }
        }
    }
    async validateTargetPath() {
        ...
        if (await fs.pathExists(targetPath)) {
            try {
                ...
                if (error instanceof Error) {
                    throw new Error(`パス検証中にエラーが発生しました: ${error.message}`);
                }
                throw new Error('パス検証中に不明なエラーが発生しました');
            }
        }
    }
