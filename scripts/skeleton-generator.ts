...
class SkeletonGenerator {
  ...
  private async promptOptions(): Promise<void> {
    try {
      // 入力フィールドの処理（バリデーション付き）
      const targetPath = await this.promptWithValidation(
        () => input({
      ...
      const projectName = await this.promptWithValidation(
        () => input({
      ...
      const includePrompts = await confirm({
        message: 'プロンプトファイルを含めますか？',
        default: true,
      });

      const includeInfrastructure = await confirm({
        message: 'インフラストラクチャファイルを含めますか？',
        default: false,
      });

      const customCursorRules = await confirm({
  ...
  private async promptWithValidation<T>(
    promptFn: () => Promise<T>,
    validateFn: (value: T) => T
  ): Promise<T> {
    while (true) {
      try {
        ...
        if (error instanceof Error) {
          ...
          console.log(chalk.red('❌ 入力エラーが発生しました'));
        }
        console.log(chalk.gray('もう一度入力してください...\n'));
      }
    }
  }
